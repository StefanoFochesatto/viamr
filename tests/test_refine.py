...
def test_refine_gr():
    ...
    imark, _, _ = amr.inactivemark(u, (psi, None), estimator="gradientrecovery", theta=0.5)
...
def test_refine_br():
    ...
    (imark, _, _) = amr.inactivemark(u, (psi, None), estimator="br78", res=residual, theta=0.5)
    rmesh = amr.refinesbr2D(mesh, imark)
    # VTKFile(f"result_inactivemark_refined.pvd").write(rmesh)
    rCG1, _ = amr.spaces(rmesh)
    assert rCG1.dim() == 109
...
def test_refine_br_total():
    ...
    u = Function(CG1).interpolate(conditional(psi > 0.0, psi + 1.0, psi))
    residual = -div(grad(u))  # largest near circle psi==0
    (imark, _, _) = amr.inactivemark(u, (psi, None), estimator="br78", res=residual, theta=0.8, method="total")
    rmesh = amr.refinesbr2D(mesh, imark)
    rCG1, _ = amr.spaces(rmesh)
    assert rCG1.dim() == 109
...
def test_refine_br_weighted():
    ...
    (imark, eta, tot_eta) = amr.inactivemark(
        u, (psi, None), estimator="bv00", res=residual, alpha=alpha, theta=0.5
    )
    # weighting by a non-constant alpha should differ from the "br78" estimator
    (imark0, eta0, tot_eta0) = amr.inactivemark(u, (psi, None), estimator="br78", res=residual, theta=0.5)
...
def test_inactivemark_badargs():
    ...
    with pytest.raises(ValueError):
        amr.inactivemark(u, (psi, None), estimator="br78")  # needs res
    with pytest.raises(ValueError):
        amr.inactivemark(u, (psi, None), estimator="br78", res=residual, alpha=alpha)  # takes no alpha
    with pytest.raises(ValueError):
        amr.inactivemark(u, (psi, None), estimator="gradientrecovery", res=residual)
    with pytest.raises(ValueError):
        amr.inactivemark(u, (psi, None), estimator="bogus")
...
def test_nsv03mark_allinactive():
    ...
    tmark, ieta, tot2 = amr.inactivemark(
        u, (lb, None), estimator="gradientrecovery", theta=0.5, method="total"
    )
